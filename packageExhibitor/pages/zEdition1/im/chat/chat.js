...
const {uploadProfileIm_exhibitor, uploadProfileIm_guanz} = require('../../../../../utils/util')
...
Page({
  ...
  onShow() {
    ...
    getApp().loginim().then(res => {
      console.log("== getApp().loginim().then(res =>=====>",)
  ...
  onLoad: function (options) {
    this.setData({
      shoot: getString('exhibitors_index', 'app.im.shoot'),
      picture: getString('exhibitors_index', 'app.im.picture'),
      send: getString('exhibitors_index', 'app.im.send'),
      options: options,
    })
    this.monitorImSDK();
  ...
  monitorImSDK() {
    ...
    wx.$app.on(TIM.EVENT.SDK_READY, this.SDK_READY); //监听SDK收到新消息

  },
  SDK_READY() {
    let data = null;
    if (this.data.options) {
      data = JSON.parse(this.data.options.data)
    }
    console.log("===SDK_READY() {====>", data.conversationID)
    ...
    wx.$app.getMessageList({
      ...
      .then(res => {
        console.log("==   wx.$app.getMessageList({=====>", res.data.messageList)
    ...
    if (eventid.startsWith('26_')) {
      ...
      wx.$app.sendMessage(message).then(res => {
        if (res.code == 0) {
          ...
          var lists = this.data.currentMessageList
          let currentMessageList = [...lists]
  ...
  enterRoom: function () {
    ...
    // wx.$app.on(TIM.EVENT.SDK_READY, onReadyStateUpdate, this) // 监听是否进入 ready 状态
    // var that = this
    // console.log(that.data.conversationData.toId)
    // var message = wx.$app.createCustomMessage({
    // 	to: that.data.conversationData.toId,
    ...
    // const url =
    // 	`../room/room?roomID=${roomID}&template=${this.data.template}&debugMode=${this.data.debugMode}&cloudenv=${this.data.cloudenv}`
    // wx.navigateTo({
    // 	url: url,
    // })
    const _this = this
  ...
  sendMessage: function () {
    ...
    console.log(projectId, '=======projectId======')
    wx.request({
      url: url + '/api3/backend/v1/statistics/addInstantMessageNum/' + projectId + '?type=chat',
      method: "GET",
      data: {},
      header: {
        'Content-Type': 'application/json'
    ...
    if (!this.isnull(this.data.messageContent)) {
      ...
      wx.showLoading({title: "发送中", duration: 4000})
      wx.$app.sendMessage(message).then(res => {
        wx.hideLoading({title: "发送中", duration: 4000})
        if (res.code == 0) {
          res.data.message.nick = app.globalData.imProfile.nick // 将个人昵称赋值给message
          res.data.message.avatar = app.globalData.imProfile.avatar // 将个人头像赋值给message
