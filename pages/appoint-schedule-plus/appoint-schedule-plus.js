...
import {myself_left, myself_schedule} from "../../common/staticImageContants";
    ...
    success: function (res) {
      ...
      if (res.data.code == '0') {
        ...
        if (userType == constant.ROLE_TYPE.EXHIBITOR) {
          ...
          that.setData({
            exhibitorConfirmed: res.data.result.invite[1].confirmed ? res.data.result.invite[1].confirmed : '0',
            exhibitorTobeconfirmed: res.data.result.invite[1].tobeconfirmed ? res.data.result.invite[1].tobeconfirmed : '0',
            exhibitorRefuse: res.data.result.invite[1].refuse ? res.data.result.invite[1].refuse : '0',
            exhibitorCancel: res.data.result.invite[1].cancel ? res.data.result.invite[1].cancel : '0',

            purchaserConfirmed: res.data.result.invite[0].confirmed ? res.data.result.invite[0].confirmed : '0',
            purchaserTobeconfirmed: res.data.result.invite[0].tobeconfirmed ? res.data.result.invite[0].tobeconfirmed : '0',
            purchaserRefuse: res.data.result.invite[0].refuse ? res.data.result.invite[0].refuse : '0',
            purchaserCancel: res.data.result.invite[0].cancel ? res.data.result.invite[0].cancel : '0',
            schedule: res.data.result.schedule
          })
        }
      } else {
...
go2rili() {
  var userType = storage.getRoleType();
  if (userType == constant.ROLE_TYPE.EXHIBITOR) {
    var url = "/packageExhibitor/pages/zEdition1/me/myInvitation/index?type=1"
  } else {
    var url = "/packagePurchaser/pages/purchaser/me/appoint-schedule/appoint-schedule?type=2"
  }
  wx.navigateTo({
    url
  })
